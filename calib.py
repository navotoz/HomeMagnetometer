...
from devices import Magnetometer
import numpy as np
...
def mag_cal():
    ...
    for qq, ax_qq in enumerate(mag_cal_axes):
        ...
        mag_array = []
        t0 = time_ns()
        with tqdm(desc=f'Rotate around {ax_qq} axis') as progressbar:
            while time_ns() - t0 < TIME_CALIB_NSEC:
                # try:
        ...
        print("Sample Rate: {0:2.0f} Hz".format(len(mag_array) / (1e-9 * (time_ns() - t0))))

    # mag_cal_rotation_vec = np.array(mag_cal_rotation_vec)  # make numpy array
    ak_fit_coeffs = []
    indices_to_save = [0, 0, 1]  # indices to save as offsets
    for mag_ii, mags in tqdm(enumerate(mag_cal_rotation_vec)):
...
def mag_cal_plot():
    ...
    measurements = (np.nanmean([x for x, y, z in measurements]),
                    np.nanmean([y for x, y, z in measurements]),
                    np.nanmean([z for x, y, z in measurements]))

    print(1e-3 * MuTESLA2MilliGAUSS(np.sqrt(((np.array(measurements) - mag_coeffs)**2).sum())))
